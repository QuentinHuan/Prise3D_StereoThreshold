...
def compute_thresholds(resultFilePath):
    ...
    for i in range(16):
        ...
        #print("parameters = ")
        #print(int(params[1]))
        T.append((params[0],int(params[1])))
    return T
...
def show_thresholdImage(resultFilePath,imgDataBasePath,bStereo):
    T = compute_thresholds(resultFilePath)
    print("----------------------------")
    ...
    for i in range(4):
        for j in range(4):
            side="right"
            im_l = Image.open(imgDataBasePath+"/p3d_"+sceneName+"-"+side+"/p3d_"+sceneName+"-"+side+"_"+  str(T[4*i + j][1]).zfill(5) +".png")
            ...
            if(bStereo==True):
                side="left"
                im_r = Image.open(imgDataBasePath+"/p3d_"+sceneName+"-"+side+"/p3d_"+sceneName+"-"+side+"_"+  str(T[4*i + j][1]).zfill(5) +".png")
                region_r = im_r.crop((i*200, j*200, (i+1)*200, (j+1)*200))
    
                imgOut.paste(region_r,(i*200+800, j*200))
    imgOut.save("./img/Thresh_"+sceneName+".png")
    print("image saved to: "+"./img/Thresh_"+sceneName+".png")